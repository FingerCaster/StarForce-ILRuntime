...
namespace UGFExtensions.Timer
{
    public class TimerComponent : GameFrameworkComponent
    {
        ...
        /// <returns></returns>
        public async Task<bool> OnceTimerAsync(long time,Action<long> updateCallBack = null, CancellationToken cancellationToken = null)
        {
            long nowTime = TimerTimeUtility.Now();
            if (time <= 0)
            {
            ...
            Timer timer = Timer.Create(time, nowTime, TimerType.OnceWait, tcs,updateCallBack: updateCallBack);
            m_Timers.Add(timer.ID, timer);
        ...
        {
            return await OnceTimerAsync(1, cancellationToken:cancellationToken);
        }
