...
namespace UGFExtensions.Hotfix
{
    ...
    public sealed class FsmManager
    {
        ...
        public void Update(float elapseSeconds, float realElapseSeconds)
        {
            ...
            for (var i = 0; i < m_FsmList.Count; i++) m_TempFsms.Add(m_FsmList[i]);

            for (var i = 0; i < m_TempFsms.Count; i++)
            {
                var fsm = m_TempFsms[i];
        ...
        {
            if (ownerType == null) throw new GameFrameworkException("Owner type is invalid.");

            return InternalHasFsm(TextUtility.GetFullName(ownerType, string.Empty));
        }
        ...
        public bool HasFsm(Type ownerType, string name)
        {
            ...
            return InternalHasFsm(TextUtility.GetFullName(ownerType, name));
        }
        ...
        public FsmBase GetFsm(Type ownerType)
        {
            ...
            return InternalGetFsm(TextUtility.GetFullName(ownerType, string.Empty));
        }
        ...
        public FsmBase GetFsm(Type ownerType, string name)
        {
            ...
            return InternalGetFsm(TextUtility.GetFullName(ownerType, name));
        }
        ...
        public IFsm CreateFsm(string name, object owner, params FsmState[] states)
        {
            if (HasFsm(owner.GetType(), name))
                throw new GameFrameworkException(Utility.Text.Format("Already exist FSM '{0}'.",
                    TextUtility.GetFullName(owner.GetType(), name)));

            var fsm = new Fsm(name, owner, states);
            m_FsmList.Add(fsm);
            m_Fsms.Add(TextUtility.GetFullName(owner.GetType(), name), fsm);
            return fsm;
        }
        ...
        public bool DestroyFsm(Type ownerType)
        {
            ...
            return InternalDestroyFsm(TextUtility.GetFullName(ownerType, string.Empty));
        }
        ...
        public bool DestroyFsm(Type ownerType, string name)
        {
            ...
            return InternalDestroyFsm(TextUtility.GetFullName(ownerType, name));
        }
        ...
        {
            if (fsm == null) throw new GameFrameworkException("FSM is invalid.");

            return InternalDestroyFsm(TextUtility.GetFullName(fsm.Owner.GetType(), fsm.Name));
        }
        ...
        public bool DestroyFsm(FsmBase fsm)
        {
            ...
            return InternalDestroyFsm(TextUtility.GetFullName(fsm.OwnerType, fsm.Name));
        }
